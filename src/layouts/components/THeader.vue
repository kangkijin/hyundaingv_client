<template>
	...
	<header id="header" class="header_wrap">
		<div class="header_top">
			<button type="button" class="btn_menu" title="menu open">MENU OPEN</button>
			<h1 class="header_logo">
				<a href="javascript:void(0);" tabindex="0">미래R&D인재 육성프로그램</a>
			</h1>
			<div class="header_info">
				<div class="name">
					<div class="img_wrap xs">
						<!-- <img src="../images/img_myinfo.png" alt="이미지 없음"> -->
					</div>
					<span>{{ user_info.krName }}</span>님
				</div>
				<div class="date">
					<router-link
						:to="{ path: user_info.role.code === 'ROLE_02' ? SCREEN.myPage.path : SCREEN.myPageClient.path }">
					</router-link><span>{{ dateLogin }}</span>
				</div>
				<div class="buttons">
					<button type="button" class="btn_sm btn_gray" @click="logout">로그아웃</button>
					<!-- <button type="button" class="btn_sm btn_gray">마이페이지</button> -->
				</div>
			</div>
		</div>
		<nav class="nav_wrap" tabindex="0">
			<ul class="gnb_1depth">
		...
		<div class="gnb_submenu">
			<div class="gnb_list" v-for=" submenu in subMenus">
				<ul class="gnb_2depth">
		...
		<!-- <div class="gnb_submenu" v-if="code == 'ROLE_03' || code == 'ROLE_02'">
			<div class="gnb_list">
			</div>
			<div class="gnb_list">
			</div>
				...
				<ul class="gnb_2depth">
					<li>
						<router-link to="/banner">
							<span>연구장학생</span></router-link>
					</li>
					<li>
						<router-link to="/banner">
							<span>계약학과</span></router-link>
					</li>
				</ul>
			</div>
			<div class="gnb_list">
...
import { getUserInfo, setUserInfo, removeUserInfo, removeAccessToken } from '@/utils/cookie'
...
const dateLogin = `${currentDate.getFullYear()}.${currentDate.getMonth() + 1}.${currentDate.getDate()} ${currentDate.getHours()}.${currentDate.getMinutes()}`;
...
onMounted(() => {
	code.value = user_info?.role.code;
	let newMenu = user_info?.role.menus ?? [];
	menus.value = newMenu.filter(item => item.parentId === null).sort((a, b) => a.displayOrder - b.displayOrder);
	let submenuAlls = newMenu.filter(item => item.parentId !== null);
	menus.value.forEach(item => {
		let newSubMenu = [];
		submenuAlls.forEach(subItem => {
		...
		if (newSubMenu.length > 0) {
			subMenus.value.push(newSubMenu.sort((a, b) => a.displayOrder - b.displayOrder));
		} else {
			subMenus.value.push([{ name: '', route: '' }])
		}
	})
	setTimeout(() => {
		headerStyle();
